...
tests = [
    ...
    ("ls", "README +'best sitcoms.txt'[ \t]+'seven dwarfs.txt'[ \t]+shells.txt", 1),
    ("ls *.txt", "'best sitcoms.txt'[ \t]+'seven dwarfs.txt'[ \t]+shells.txt", 1),
    ("echo $PATH", "\\$PATH", 1),
    ("author", "", 1),
    ('author | sed -e "s/^/Written by /"', "Written by ", 1),
    ("grep Happy *.txt", "best sitcoms.txt:Happy Days.*seven dwarfs.txt:Happy", 1),
    ('cat "best sitcoms.txt" | grep Seinfeld', "Seinfeld", 1),
    ('cat "best sitcoms.txt"|grep Seinfeld|wc -l', "1", 1),
    ('sed -ne "s/The Simpsons/I Love Lucy/p" < best\\ sitcoms.txt > output', "", 1),
    ("ls -l", "output", 1),
    ("cat output", "I Love Lucy", 1),
    (
        "this is not a command",
        "this: Command not found.*Child.*exited with status 2",
        2,
    ),
    ("echo Hello > /usr/bin/cant_write", "/usr/bin/cant_write: Permission denied", 2),
    ("cd", "", 1),
